...
public class IntArrayBinSearchPool
{
  ...
  /** 
   * remove partition from pool and close it
   * @param term term that uses partition
   */
  public void removePartitionForTerm(String term)
    throws IOException
  {
    String partitionId = getPartitionId(term);
    this.remove(partitionId);
  } 

  /**
   * remove partition from pool and close it 
   * @param partitionId label for partition.
   */
