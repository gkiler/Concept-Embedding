#!/bin/sh

# Usage: script -c "./runTest_2014.sh -TPD" typescript.runTest_2014.sh
# That is the default call.
# Be sure to set METAMAP_TEST and METAMAP_PROD just below the comments
# Set METAMAP_PROD here!!
# and
# Set METAMAP_TEST here!!

announce() {
   echo $1
}

split_line(){
   LINE=$1
   FILENAME=`echo $LINE | cut -d'|' -f1`
   PRODFLAGS=`echo $LINE | cut -d'|' -f2`
   TESTFLAGS=`echo $LINE | cut -d'|' -f3`
   TITLE=`echo $LINE | cut -d'|' -f4`
   MODIFY=`echo $LINE | cut -d'|' -f5`

   # echo "LINE     = $LINE"
   # echo "FILENAME = $FILENAME"
   # echo "FLAGS    = $FLAGS"
   # echo "TITLE    = $TITLE"
   # echo "MODIFY   = $MODIFY"
}

test_filename_exists() {
   if [ ! -f $1 ]
   then
      echo ERROR: filename $1 does not exist.
      exit 1
   fi
}

change_metamap_text() {

   MODIFY=$1
   METAMAP=$2
   ORIG_FILENAME=$3
   NEW_FILENAME=$4

   if [ $MODIFY -eq 1 ]
   then
      echo "changing |$METAMAP|to|MetaMap|"

      sed -e "s#$METAMAP#MetaMap#g" $ORIG_FILENAME > $NEW_FILENAME
      /bin/rm -f $ORIG_FILENAME
   else
      /bin/mv -f $ORIG_FILENAME $NEW_FILENAME
   fi
}

diff_prod_test_files(){
   PROD_FILENAME=$1
   TEST_FILENAME=$2
   DIFF_FILENAME=$3

   if [ ! -f $PROD_FILENAME ]
   then
      echo "prod result file $PROD_FILENAME does not exist"
      exit 1
   elif [ ! -f $TEST_FILENAME ]
   then
      echo "test result file $TEST_FILENAME does not exist"
      exit 1
   else
      diff -b $PROD_FILENAME $TEST_FILENAME > $DIFF_FILENAME
      if [ -s $DIFF_FILENAME ]
      then
         cat $DIFF_FILENAME
      else
         echo DIFFS OK
      fi
   fi
}

run_MetaMap() {
   PRODorDEVL=$1
   PROGRAM=$2
   FLAGS=$3
   FILENAME=$4
   MODIFY=$5

   INFILE=$FILENAME.txt
   TMP_OUTFILE=$FILENAME.${PRODorDEVL}.OUT
   OUTFILE=$FILENAME.${PRODorDEVL}

   COMMAND="$PROGRAM $FLAGS $INFILE $TMP_OUTFILE"
   echo $COMMAND
   $COMMAND
#    echo "cat $INFILE | $PROGRAM $FLAGS > $TMP_OUTFILE 2>&1 "
#    cat $INFILE | $PROGRAM $FLAGS > $TMP_OUTFILE 2>&1 

#   if [ ! -s $TMP_OUTFILE ]
#   then
#      echo ERROR: $TMP_OUTFILE is zero length.
#      exit 1
#   fi
   mv $TMP_OUTFILE $OUTFILE
#   change_metamap_text $MODIFY "$PROGRAM" $TMP_OUTFILE $OUTFILE
}

RUN_PROD=0
RUN_TEST=0
RUN_DIFF=0
PROGRAM=`basename $0`
USAGE='Usage: $PROGRAM [ -P -T ]'

while getopts TPD option
do
   case $option
   in
   	P) RUN_PROD=1;;
   	T) RUN_TEST=1;;
   	D) RUN_DIFF=1;;
        *) exit 1;
   esac
done

shift `expr $OPTIND - 1`
OPTIND=1

# The various tests are defined in lines such as the following:
# allow_overmatches|-o|-o|Overmatches|0|
# prefer_multiple_concepts|-Y|-Y|Prefer Multiple Concepts|0|
# ignore_word_order|-i|-i|Ignore Word Order|0|
#
#         FILENAME | PRODFLAGS | TESTFLAGS | TITLE | MODIFY
#
# FILENAME:   The name of the file containing the text to be tested; assumes a ".txt" extension.
# PROD-FLAGS: The flags passed to Production MetaMap to run this test.
# DEVL-FLAGS: The flags passed to Development MetaMap to run this test.
# TITLE:      The name of the test (human readable).
# MODIFY:     0 or 1, depending on whether "MetaMap" should replace $METAMAP in the output file.
#             MODIFY should be 1 iff the test in question generates MMO or XML
# Unfortunately, the MODIFY feature needs some work...

# Set METAMAP_PROD here!!
# METAMAP_PROD is the current production version
# METAMAP_PROD="metamap.FML      -Z 2017AB -L 2017"
METAMAP_PROD="metamap      -Z 2018AA -L 2018"

# Set METAMAP_TEST here!!
# METAMAP_TEST is the new test version
# METAMAP_TEST="metamap.FML      -Z 2018AA -L 2018"
METAMAP_TEST="metamap      -Z 2018AB -L 2018"

echo "Start Time: `date`"
echo ""

if [ $RUN_PROD -eq 1 ]
then
  echo "MetaMap PROD: $METAMAP_PROD"
fi

if [ $RUN_TEST -eq 1 ]
then
   echo "MetaMap TEST: $METAMAP_TEST"
fi
echo ""

while read LINE
do
   echo Testing $LINE

   # This call to split_line sets the five variables
   # FILENAME, PRODFLAGS, TESTFLAGS, TITLE and MODIFY
   # FILENAME | PRODFLAGS | TESTFLAGS | TITLE| MODIFY

   split_line "$LINE"

   # for ease of maintenance, allow TESTFLAGS to be empty,
   # in which case use PRODFLAGS as TESTFLAGS
   if [ "$TESTFLAGS" == "" ]
   then
       TESTFLAGS=$PRODFLAGS
   fi

   test_filename_exists $FILENAME.txt

   if [ $RUN_PROD -eq 1 ]
   then
      echo running run_MetaMap prod "$METAMAP_PROD" "$PRODFLAGS" $FILENAME $MODIFY
      run_MetaMap prod "$METAMAP_PROD" "$PRODFLAGS -E" $FILENAME $MODIFY
   fi

   if [ $RUN_TEST -eq 1 ]
   then
      echo running run_MetaMap test "$METAMAP_TEST" "$TESTFLAGS" $FILENAME $MODIFY
      run_MetaMap test "$METAMAP_TEST" "$TESTFLAGS -E" $FILENAME $MODIFY
   fi

   if [ $RUN_DIFF -eq 1 ]
   then
      echo "====================== DIFFS for $FILENAME ============================="
      diff_prod_test_files $FILENAME.prod $FILENAME.test $FILENAME.diff
      echo "====================== DIFFS END ==============================="
      echo ""
   fi
done << EOF
5zb|-zbI --debug 5|-zbI --debug 5|5zb Option|0|
AAs2000|--aas_only|--aas_only|AAs2000|0|
AAsTest|--aas_only|--aas_only|AAs Test|0|
EOT_marker|||EOT marker|0|
ExcludeNLMSubSynRelaxed|-e NLMSubSyn -C|-e NLMSubSyn -C|ExcludeNLMSubSynRelaxed|0|
ExcludeNLMSubSynStrict|-e NLMSubSyn|-e NLMSubSyn|ExcludeNLMSubSynStrict|0|
JSON_format|--JSONf 2|--JSONf 2|JSON format|0|
MMI_fielded_restrict_to_sources|-N -R MSH|-N -R MSH|MMI Fielded Output, Restrict to Sources|0|
MMI_fielded|-N|-N|MMI Fielded Output|0|
MMO|-q|-q|Machine Output|1|
MarineCorps|||Marine Corps|0|
NLMSubSynNoFlagsRelaxed|-C|-C|NLMSubSynNoFlagsRelaxed|0|
NLMSubSynNoFlagsStrict|||NLMSubSynNoFlagsStrict|0|
NegEx_HR|--negex -Q 1|--negex -Q 1|NegEx HR Output|1|
NegEx_MMO|-q|-q|Simple NegEx Machine Output|1|
NegEx_XML|--XMLf|--XMLf|NegEx, XML format|1|
RestrictToNLMSubSynRelaxed|-R NLMSubSyn -C|-R NLMSubSyn -C|RestrictToNLMSubSynRelaxed|0|
RestrictToNLMSubSynStrict|-R NLMSubSyn|-R NLMSubSyn|RestrictToNLMSubSynStrict|0|
UDA|--UDA UDAfile|--UDA UDAfile|UDA|0|
WSD_off_1|||Without WSD 1|0|
WSD_off_2|||Without WSD 2|0|
WSD_on_1|-y|-y|With WSD 1|0|
WSD_on_2|-y|-y|With WSD 2|0|
XML_format1|--XMLf1|--XMLf1|XML Format1|1|
XML_format|--XMLf|--XMLf|XML format|1|
aged|-D|-D|aged|1|
allow_acros_abbrs_MMO|-aq|-aq|Acros and Abbrvs, MMO|1|
allow_concept_gaps|-g|-g|Concept Gaps|0|
allow_overmatches|-CozmicIn|-CozmicIn|Overmatches|0|
apostrophe_s_normal|||apostrophe_s_normal|0|
best_mappings_only|||Normal Processing - Best Mappings Only|0|
cascadeOFF|-J dsyn|-J dsyn|cascadeOFF|0|
cascadeON|-J dsyn --cascade|-J dsyn --cascade|cascadeON|0|
composite_phrases|-Q 5 -V NLM|-Q 5 -V NLM|Composite Phrases|0|
compute_all_mappings_2|-bn|-bn|All Mappings 2|0|
compute_all_mappings_debug5|-bf --debug 5|-bf --debug 5|All Mappings, Debug5|0|
compute_all_mappings_threshhold_900|-nbr 900|-nbr 900|All Mappings, Threshold=900|0|
compute_all_mappings|-bn|-bn|All Mappings|0|
conj|--conj -I|--conj -I|conj|0|
exclude_sources|-e MSH|-e MSH|Exclude Sources|0|
fielded_MMI_ignore_word_order|-Ni|-Ni|Fielded MMI Output, Ignore Word Order|0|
head_scoring_1|-z|-z|Head Scoring 1|0|
head_scoring_2|-zi|-zi|Head Scoring 2|0|
hide_mappings|-mc|-mc|Hide Mappings, Show Candidates|0|
ignore_word_order_term_processing|-zi|-zi|Ignore Word Order Term Processing|0|
ignore_word_order|-i|-i|Ignore Word Order|0|
ivcsf|||ivcsf|0|
model_Base|-V Base -Isf|-V Base -Isf|Model Base|0|
model_NLM|-V NLM -Isf|-V NLM -Isf|Model NLM|0|
model_USAbase|-V USAbase -Isf|-V USAbase -Isf|Model USAbase|0|
no_hide_semantic_types|||No Hide Semantic Types|0|
no_numsOFF|||no nums OFF|0|
no_numsON|--no_nums ALL|--no_nums ALL|no nums ON|0|
nomapOFF|||nomapOFF|0|
nomapON|--nomap NoMapFile|--nomap NoMapFile|nomapON|0|
non_standard_PMID|||Non-standard PMID|0|
novarOFF|-Dv|-Dv|novarOFF|0|
novarON|-DIcv --novar NoVarFile|-DIcv --novar NoVarFile|novar|0|
num_breakOFF|||num break OFF|0|
num_breakON|--num_break|--num_break|num break ON|0|
plain_vanilla|||Plain Vanilla|0|
prefer_multiple_concepts|-Y|-Y|Prefer Multiple Concepts|0|
pruning|--prune 10|--prune 10|Pruning|0|
restrict_to_sources_XML|-R MSH --XMLf|-R MSH --XMLf|Restrict to Sources, XML format|1|
short_semantic_types|-s|-s|Short Semantic Types|0|
show_candidates|-c|-c|Show Candidates|0|
term_processing|-z|-z|Term processing|0|
EOF
